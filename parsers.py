...
class SequenceParser(object):
    ...
    def __init__(self, name=None):
        # 64, 56, 48, 40, 32, 28, 24, 20, 16, 14, 12, 10, 8, 6, 4, 2 
        self.durations = [1, 7/8, 3/4, 5/8, 1/2, 7/16, 3/8, 5/16, 1/4, 7/32, 3/16, 5/32, 1/8, 3/32, 1/16, 1/32]
        self.name = name
        pass
    ...
    def get_monosyllabic_melody(self, sequence):
        sequence = np.array(sequence)
        span = sequence.shape[-1]
        notes = self.get_notes(sequence)
        notes = self.sort_notes(notes, mode=1, reverse=False)
